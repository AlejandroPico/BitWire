...
function connectedPins(capture:InstrumentCapture){return capture.pins.filter(pin=>pin.wireId||pin.wireLabel?.startsWith('Vínculo interno')).length;}
function primaryPin(capture:InstrumentCapture){return capture.pins.find(pin=>(pin.wireId||pin.wireLabel)&&pin.pinId!=='gnd')??capture.pins[0];}
...
function reading(entry:InstrumentEntry){const capture=entry.capture,type=entry.definition.id;if(type==='frequency_counter')return formatFrequency(capture.frequency);if(type==='power_monitor'||type==='wattmeter')return `${capture.power.toFixed(3)} W`;if(type==='ammeter')return `${capture.current.toFixed(6)} A`;if(type==='ohmmeter')return capture.resistance===undefined?'— Ω':`${capture.resistance.toFixed(3)} Ω`;if(type==='logic_analyzer'||type==='logic_output')return `D ${primaryPin(capture)?.logic.at(-1)??'Z'}`;if(type==='spectrum_analyzer')return `${capture.maximum.toFixed(2)} Vpk`;return `${capture.voltage.toFixed(3)} V`;}
function secondaryReading(entry:InstrumentEntry){const capture=entry.capture,type=entry.definition.id;if(type==='frequency_counter')return `Periodo ${formatPeriod(capture.period)}`;if(type==='power_monitor'||type==='wattmeter')return `Energía ${capture.energy.toFixed(4)} J`;if(type==='ammeter')return `Valor absoluto ${Math.abs(capture.current*1000).toFixed(3)} mA`;if(type==='ohmmeter')return 'Sonda virtual de alta impedancia';if(type==='multimeter')return `I ${Math.abs(capture.current*1000).toFixed(3)} mA · RMS ${capture.rms.toFixed(3)} V`;return `Mín ${capture.minimum.toFixed(2)} · Máx ${capture.maximum.toFixed(2)}`;}
function metricLabel(type:string){if(type==='frequency_counter')return 'FRECUENCIA';if(type==='power_monitor'||type==='wattmeter')return 'POTENCIA INSTANTÁNEA';if(type==='ammeter')return 'CORRIENTE';if(type==='ohmmeter')return 'RESISTENCIA';if(type==='multimeter')return 'MEDIDA PRINCIPAL';return 'LECTURA DE SONDA';}
